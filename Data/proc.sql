...
BEGIN  
 ...
 BEGIN  
  RAISERROR('Invalid parameter: Provided IdProduct does not exist', 15, 0);  
  COMMIT
  RETURN;
 END;  
 ...
 IF @IdOrder IS NULL  
 BEGIN  
  RAISERROR('Invalid parameter: There is no order to fullfill', 15, 0);  
  COMMIT
  RETURN;
 END;  
 ...
 BEGIN  
  RAISERROR('Invalid parameter: Provided IdWarehouse does not exist', 15, 0); 
  COMMIT
  RETURN;
 END;  
